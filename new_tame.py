...
class TameLinear(torch.nn.Module):
    def __init__(self,
        ...
        self.A = torch.nn.Parameter(torch.randn([output_width, input_width], dtype=dtype, device=device))
...
class TameNetwork(torch.nn.Module):
    ...
    def forward(self, X):
        X1 = X * self.input_scales.view(-1, 1)
        return self.layer(X1)[(-self.output_width):, :]

    def penalty(self):
        return self.l2_input * torch.sum(self.input_scales ** 2) + self.layer.penalty()
        # return self.l2_input * torch.sum(torch.abs(self.input_scales)) + self.layer.penalty()
    ...
    if i % 1 == 0:
        with torch.no_grad():
            ...
            if optimizer.state['f'] != obj:
                ...
                logging.info("iter={}: obj={:.8f}, train={:.8f}, test={:.8f}, auc={:.8f}, grad={:.7g}, scale={:.4f}, tr_radius={:.3g}".format(
