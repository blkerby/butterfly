            ...
            for(;;) {
                ...
                for (long row_idx = 0; row_idx < num_rows; row_idx += BATCH_SIZE) {
                    ...
                    for (int layer = 0; layer < NUM_BLOCK_LAYERS; layer++) {
                        long a = NUM_BLOCK_ANGLES * col_block + COL_BLOCK_WIDTH / 2 * layer;
    ...
    for (int w = 0; w < NUM_WORKERS; w++) {
        workers.push_back(thread([=, &next_col_block] {
            ...
            for(;;) {
                ...
                // intermediate results fit in cache)
                for (long row_idx = 0; row_idx < num_rows; row_idx += BATCH_SIZE) {
                    int stride = COL_BLOCK_WIDTH;
                    int stride_pow = COL_BLOCK_WIDTH_POW;
                    
                    // Iterate over the butterfly layers within this given column block and row batch:
                    for (int layer = NUM_BLOCK_LAYERS - 1; layer >= 0; layer--) {
                        if (stride_pow == 0) {
...
void butterfly_backward(at::Tensor data, at::Tensor grad_data, at::Tensor angles, at::Tensor grad_angles, at::Tensor indices_in, int idx_out) {
    AT_DISPATCH_FLOATING_TYPES(data.type(), "butterfly_forward", ([&] {
        ...
        cpu_butterfly_backward(data_ptr, grad_data_ptr, indices_in_ptr, idx_out, angles_ptr, grad_angles_ptr, num_rows, col_stride, num_col_blocks);
    }));
}
