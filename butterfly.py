...
import math
from butterfly_cuda import OrthogonalButterfly
...
#     def __init__(self, width, penalty_bias, penalty_slope, penalty_curvature, dtype=torch.float, device=None):
#         super().__init__()
#         self.dtype = dtype
...
#         eps = 0.01
#         self.bias = torch.nn.Parameter(torch.rand([width], dtype=dtype, device=device) * 2 * eps - 1)
#         self.slope_1 = torch.nn.Parameter(torch.rand([width], dtype=dtype, device=device) * 2 * eps - 1)
#         self.slope_2 = torch.nn.Parameter(torch.rand([width], dtype=dtype, device=device) * 2 * eps - 1)
#         self.curvature = torch.nn.Parameter(torch.rand([width], dtype=dtype, device=device) * 2 * eps - 1)
...
#     def __init__(self, width, l2_slope, l2_scale, l2_bias, dtype=torch.float, device=None):
#         super().__init__()
#         self.dtype = dtype
...
#         eps = 0.1
#         self.bias = torch.nn.Parameter(torch.rand([width], dtype=dtype, device=device) * 2 * eps - eps)
#         self.slope = torch.nn.Parameter(torch.rand([width], dtype=dtype, device=device) * 2 * eps - eps + 1)
#         self.scale = torch.nn.Parameter(torch.rand([width], dtype=dtype, device=device))
...
class CubicSquashActivation(torch.nn.Module):
    ...
    def forward(self, X):
        assert X.dtype == self.dtype
        u = torch.clamp((X * self.slope.view(-1, 1) - self.bias.view(-1, 1)) / self.scale.view(-1, 1), -1, 1)
        return self.scale.view(-1, 1) * (1.5 * u - 0.5 * u ** 3)
...
class QuinticSquashActivation(torch.nn.Module):
    def __init__(self, width, l2_slope, l2_scale, l2_bias, dtype=torch.float, device=None):
        ...
        self.bias = torch.nn.Parameter(torch.rand([width], dtype=dtype, device=device) * 2 * eps - eps)
        self.slope = torch.nn.Parameter(torch.rand([width], dtype=dtype, device=device) * 2 * eps - eps + 1)
        self.scale = torch.nn.Parameter(torch.rand([width], dtype=dtype, device=device) * 2 * eps - eps + 1)

    def forward(self, X):
        assert X.dtype == self.dtype
        u = torch.clamp((X * self.slope.view(-1, 1) - self.bias.view(-1, 1)) / self.scale.view(-1, 1), -1, 1)
        return self.scale.view(-1, 1) * (1.875 * u - 1.25 * u ** 3 + 0.375 * u ** 5)
...
class CustomNetwork(torch.nn.Module):
    def __init__(self, num_inputs, num_outputs, width_pow, depth, butterfly_depth,
                 l2_slope, l2_scale, l2_bias, l2_interact, dtype=torch.float, device=None):
        super().__init__()
        self.dtype = dtype
        ...
        for i in range(depth):
            butterfly = OrthogonalButterfly(width_pow, butterfly_depth, l2_interact, dtype=dtype, device=device)
            # activation = QuinticSquashActivation(self.width, l2_slope, l2_scale, l2_bias, dtype=dtype, device=device)
            activation = CubicSquashActivation(self.width, l2_slope, l2_scale, l2_bias, dtype=dtype, device=device)
            # activation = SmoothSquashActivation(self.width, l2_slope, l2_scale, l2_bias, dtype=dtype, device=device)
            # activation = SmoothBendActivation(self.width, l2_bias, l2_slope, l2_scale, dtype=torch.float, device=device)
    ...
    def forward(self, X):
        ...
        X_in = torch.cat([X, torch.zeros([self.width - self.num_inputs, X.shape[1]], dtype=self.dtype, device=X.device)], axis=0)
